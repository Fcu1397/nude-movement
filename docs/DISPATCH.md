...
| [G3](tasks/G3-visual-qa.md) | 截圖視覺 QA 與修正 | Gemini | C1 C2 G1 + `npm run shots` | C3 |
...
- 環境：全機 Node 20.11 不升級，專案內以 devDependency 使用 Node 22（已在本機實測 install / generate / test / typecheck / lint 全通過），做法見 DECISIONS D1 與 C1。每個 worktree 各自安裝一次依賴。
...
- 本機全域 Node 是 20.11 且不可升級：請以任務卡中的 package.json baseline 手動建立專案，依賴安裝只能用 npx -y -p node@22.23.2 -p npm@11 -c "npm install"，不要直接 npm install、不要 nuxi init、不要降版。
- 做完跑 npm run lint && npm run typecheck && npm run test && npm run generate，全數通過後 commit。
...
- 全部完成後跑 npm run lint && npm run typecheck && npm run generate，commit，依回報格式回覆。
```
...
- 完成後跑 npm run typecheck && npm run test && npm run generate，commit，依回報格式回覆。
```
...
- 請「先」完成第 1 項 npm run shots 並 commit，回報一句「shots ready」，Gemini 會以此開始 G3。
...
- 先跑 npm run shots，再用 @.shots/<檔名>.png 逐張讀取截圖審查。
...
- [ ] `npm run lint && npm run typecheck && npm run test && npm run generate` 在合併後的 main 通過
